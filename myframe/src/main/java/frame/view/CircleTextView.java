    ...
    public CircleTextView(Context context, @Nullable AttributeSet attrs) {
        ...
        try {
            ...
            if (a.getBoolean(R.styleable.CircleTextView_randColor,true)) {
                //随机颜色
                mBgColor=getRandColor(context);
            }else {
                //自定义颜色
                mBgColor=a.getColor(R.styleable.CircleTextView_bgColor,getRandColor(context));
            }
            //半径
...
//            canvas.drawCircle(measuredWidth/2f,measuredHeight/2f,mRadius+mBorderWidth,mBorderPaint);
    ...
    public CircleTextView text(String text){
        mText=text;
        return this;
    }

    public CircleTextView bgColor(int color){
        mBgColor=color;
        return this;
    }
